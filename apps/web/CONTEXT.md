...
- 타임라인은 기본 접힘의 보조 자료로 표시해 원본·링크·참여보다 시각 비중을 낮추고, 파생·연결 관계를 다른 사전 항목보다 우선 노출한다.
- `/quiz`는 관리자가 구성한 최대 5개 카드의 인지도와 상세 설명 도움 여부를 익명 세션 단위로 측정한다. 모바일은 수평 swipe만 허용하고 완료 화면에서 진행한 항목의 상세 링크를 제공한다.
- 상세의 최근 사용 카드는 로그인 없이 `봤어요/못 봤어요`를 받아 최근 14일의 가벼운 사용 신호 그래프를 보여준다.

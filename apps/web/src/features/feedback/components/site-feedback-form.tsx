  ...
  return (
    <Card className="p-5 sm:p-7">
      <form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}>
        <Field label="문의 제목" wide>
          <input name="title" placeholder="오류, 개선 의견, 권리 문의 등" required />
        </Field>
        ...
        <Field label="내용" wide>
          <textarea name="description" placeholder="확인할 수 있도록 상황과 원하는 조치를 적어주세요." required />
        </Field>
        <div className="sm:col-span-2">
          <Button className="w-full" disabled={status === "sending"} size="lg" type="submit">
            ...
            {status === "sending" ? "보내는 중" : "운영자에게 보내기"}
          </Button>

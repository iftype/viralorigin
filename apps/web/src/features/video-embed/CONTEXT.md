...
- 한계: TikTok과 Instagram은 삭제, 비공개, 지역, 로그인 및 작성자의 임베드 설정에 따라 재생이 제한될 수 있다. Instagram이 공식 embed 응답에서 차단 상태를 반환하면 깨진 iframe 대신 원문 링크 fallback을 표시한다. X와 알 수 없는 플랫폼은 링크 카드로 표시한다.
